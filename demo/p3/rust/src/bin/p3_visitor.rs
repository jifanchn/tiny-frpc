...
impl TunnelEventHandler for VisitorHandler {
...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <server_addr> <server_port> <visitor_name>", args[0]);
        std::process::exit(1);
    }
    let host = &args[1];
    ...
    println!("Starting Rust STCP Visitor '{}' connecting to {}:{}...", name, host, port);

    let mut client = FrpcClient::new(host, port, Some("test_token"))?;
    client.set_encryption(true);

    let handler = Arc::new(VisitorHandler);

    let config = TunnelConfig {
        tunnel_type: TunnelType::StcpVisitor,
        tunnel_name: format!("visitor_{}", name),
        secret_key: Some("p3_test_secret".to_string()),
        local_addr: None,
        local_port: None,
        remote_name: Some("p3_test_stcp".to_string()),
        bind_addr: None,
        bind_port: None,
    };
