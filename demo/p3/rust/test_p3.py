#!/usr/bin/env python3
"""
P3 (Three-Process) Test for Rust Implementation
"""
...
# Rust binaries
SERVER_SCRIPT = os.path.join(SCRIPT_DIR, "target/debug/p3_server")
VISITOR_SCRIPT = os.path.join(SCRIPT_DIR, "target/debug/p3_visitor")
...
""")
    ...
    def start_server(self):
        self.pm.start_process("Server", [SERVER_SCRIPT, "127.0.0.1", str(self.port)], env=self.get_env())

    def start_visitor(self, name):
        self.pm.start_process(f"Vis-{name}", [VISITOR_SCRIPT, "127.0.0.1", str(self.port), name], env=self.get_env())
...
def test_multi_visitor():
    print("="*60)
    print("  Test: Rust Implementation (Server & Visitors)")
    print("="*60)
    ...
    try:
        ...
        print("\nPASSED: Rust implementation works!")
        return True
...
def main():
    if not os.path.exists(SERVER_SCRIPT):
        print(f"Error: Server binary not found at {SERVER_SCRIPT}")
        print("Please run 'cargo build' in demo/p3/rust/")
        return 1
    test_multi_visitor()
